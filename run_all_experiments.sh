...
conda run -n ssm python train.py --model transformer --input_type conv --epochs $EPOCHS --batch_size 64

echo -e "\n[6/11] Transformer + MFCC"
conda run -n ssm python train.py --model transformer --input_type mfcc --epochs $EPOCHS --batch_size 4096

# ── S4D (diagonal SSM, NeurIPS 2022) ─────────────────────────────────────────
echo -e "\n[7/11] S4D + Raw Waveform  (diagonal, N=64)"
conda run -n ssm python train.py --model s4 --input_type raw  --ssm_type s4d --epochs $EPOCHS --batch_size 16

echo -e "\n[8/11] S4D + Conv Frontend"
conda run -n ssm python train.py --model s4 --input_type conv --ssm_type s4d --epochs $EPOCHS --batch_size 64

echo -e "\n[9/11] S4D + MFCC"
conda run -n ssm python train.py --model s4 --input_type mfcc --ssm_type s4d --epochs $EPOCHS --batch_size 4096
